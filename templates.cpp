...
struct Point {
	...
	*/
	...
	longd len() const { return sqrt(len2()); }
	vartype len2() const { return x * x + y * y; }
	longd atan2() const { return ::atan2(y, x); }
	longd getAngle(const Point& v2) const { return (*(this) * v2) / (len() * v2.len()); }
	vartype getKFract(const Point& v2) const { return (*this * v2); }
	longd getK(const Point& v2) const { return getKFract(v2) / (longd)len2(); }
	Point projection(const Point& v2) const { return *(this) * getK(v2); }
